...
public class OrderInfoController {
    ...
    public Result<OrderInfo> add(@RequestBody OrderInfo orderInfo) {
        ...
        OrderInfo orderInfo1 = orderInfoService.add(orderInfo);
        return Result.success(orderInfo1);
    }

    @PostMapping("/groupBuy")
    public Result groupBuy(@RequestBody GroupBuy groupBuy) {
        ...
        if (buys.size() < 3) {
            throw new CustomException("-1", "人数未满不能开团");
        }

        // 提交订单
        orderInfo.setTotalPrice(groupBuy.getPrice());
        orderInfo.setUserId(groupBuy.getUserId());
        orderInfo.setStatus("待付款");
        orderInfoService.add(orderInfo);


        groupBuyDao.deleteByGoodsId(groupBuy.getUserId(), goodsId);
