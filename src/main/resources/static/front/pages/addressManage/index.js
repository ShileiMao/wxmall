...
Page({
   ...
   //获取地图 map 实例
    ...
    if (longitudeLatitude) {
      ...
      wx.startLocationUpdate({
        success: (res) => {
          wx.hideLoading();
          console.log("location update: " + JSON.stringify(res));
  ...
  onShow: function () {
    ...
    if (selectedCity) {
      ...
      console.log("selected city: " + JSON.stringify(selectedCity));
      this.setData({
...
//搜索附近地点
  ...
  selectedOk: function () {
    ...
    let postData = {userId: userInfo.id, province: province, city: city, district, district, addr: addr};
    request({url: "/updatePostAddress",
    data: postData,
    method: "POST"
    }).then(res => {
      console.log("update address result: " + JSON.stringify(res));
      if (res.code === '0') {
        console.log("update address result: " + JSON.stringify(res));
        prevPage.setData({
          district: that.data.centerData.district,
          province: that.data.centerData.province,
          city: that.data.centerData.city,
          addr: that.data.centerData.addr,
          longitude: that.data.centerData.longitude,
          latitude: that.data.centerData.latitude
        })
        prevPage.onLoad();
        console.log("center data: " + JSON.stringify(this.data.centerData));
        wx.navigateBack({
          delta: 1
        })
      }
  ...
  initLocation: function (latitude, longitude) {
    console.log("init location: " + latitude + ", longitude: " + longitude);
    qqmapsdk.reverseGeocoder({
      ...
      success: (res) => {
        ...
        this.moveToLocation.call(this, latitude, longitude);
        // 调用接口
        this.nearby_search.call(this);
      },
    });
