...
import com.example.dao.UserAddrDao;
...
public class UserInfoService {
     ...
     * 登录
     */
    public UserInfo login(String name, String password, String loginMethod) {
        List<UserInfo> list = userInfoDao.findByNameAndLoginMethod(name, loginMethod);
    ...
    public UserInfo searchExistingUser(String name, String loginMethod) {
        List<UserInfo> list = userInfoDao.findByNameAndLoginMethod(name, loginMethod);
        if (CollectionUtil.isEmpty(list)) {
            throw new CustomException(ResultCode.USER_NOT_EXIST_ERROR);
        }
    ...
    public UserAddr getPostAddress(Long id) {
        List<UserAddr> list = userAddrDao.getAddress(id);
        if (CollectionUtil.isEmpty(list)) {
            throw new CustomException(ResultCode.USER_NOT_EXIST_ERROR);
        }
    ...
    public UserAddr updateOrInsertAddress(UserAddr address) {
        Long userId = address.userId;

        try {
            UserAddr addr = getPostAddress(userId);
            ...
            if (addr != null) {
                ...
                userAddrDao.updateByPrimaryKey(addr);
//            userAddrDao.updateByPrimaryKeySelective(addr);

                return addr;
            }
        } catch (Exception e) {
            System.out.println("no post address found, insert new one");
        }
        userAddrDao.insertSelective(address);

        return getPostAddress(userId);
    }
}
